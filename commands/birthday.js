...
module.exports = {
    ...
    usage: cfg => [
        'birthday [name] [date] -\n\tIf name and date are specified, set the named ' +
            cfg.lang +
            "'s birthday to the date.\n\tIf name only is specified, show the " +
            cfg.lang +
    ...
    execute: async (bot, msg, args, cfg) => {
        ...
        if (!args[1])
            return member.birthday
                ? 'Current birthday: ' + member.birthday.toDateString() + '\nTo remove it, try ' + cfg.prefix + 'birthday ' + member.name + ' clear'
                : 'No birthday currently set for ' + args[0]
