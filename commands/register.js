...
module.exports = {
    ...
    execute: async (bot, msg, args, cfg) => {
        ...
        //check arguments
        let brackets = msg.content
            .slice(msg.content.indexOf(args[0]) + args[0].length + 1)
            .trim()
            .split('text')
        let name = args[0].trim()
        let member = await bot.db.getMember({ user_id: msg.author.id, name, brackets })
        //(
        //await bot.db.query('SELECT name,brackets FROM Members WHERE user_id = $1::VARCHAR(32) AND (LOWER(name) = LOWER($2::VARCHAR(32)) OR brackets = $3)', [msg.author.id, name, brackets || []])
        //).rows[0]
        ...
        if (brackets.length < 2)
            return "No 'text' found to detect brackets with. For the last part of your command, enter the word 'text' surrounded by any characters.\nThis determines how the bot detects if it should replace a message."
