module.exports = async (msg, bot) => {
    ...
    if (guild) cfg = (await bot.db.getCfg(guild.id)) || { prefix: 'tul!', lang: 'tupper' }
    ...
    let members = await bot.db.getMembers(msg.author.id) //(await bot.db.query('SELECT * FROM Members WHERE user_id = $1 ORDER BY position', [msg.author.id])).rows
    if (members && !(msg.channel.type == 1) && (!guild || !(await bot.db.isBlacklisted(guild.id, msg.channel.id, true)))) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!found && current) replace[replace.length - 1][3] += '\n' + lines[i]
        }
        ...
        if (replace[0]) {
            try {
                ...
                let perms = msg.channel.permissionsOf(bot.user.id)
                if (perms.has('manageMessages') && perms.has('readMessages')) await msg.delete()
            } catch (e) {
