...
module.exports = {
    ...
    usage: cfg => [
        "cfg prefix <newPrefix> - Change the bot's prefix",
        "cfg rename <newname> - Change all instances of the default name 'member' in bot replies in this server to the specified term",
        'cfg log <channel> - Enable the bot to send a log of all ' +
            cfg.lang +
            ' messages and some basic info like who registered them. Useful for having a searchable channel and for distinguishing between similar names.',
            ...
            case 'blacklist':
                ...
                switch (args[1]) {
                    case 'add':
                        ...
                        channels = args
                            .slice(2)
                            .map(arg => bot.resolveChannel(msg, arg))
                            .map(ch => {
                                if (ch) return ch.id
                                else return ch
                            })
                        ...
                        if (channels.find(ch => ch == undefined)) {
                            ...
                            for (let i = 0; i < channels.length; i++) if (!channels[i]) out += args.slice(2)[i]
                            return out
                        }
