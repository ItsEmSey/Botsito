...
module.exports = bot => {
    bot.replaceMessage = async (msg, cfg, member, content, retry = true) => {
        ...
        const data = {
            wait: true,
            content: bot.recent[msg.channel.id]
                ? content.replace(tagRegex, match => {
        ...
        try {
            ...
            if (e.code === 10015) {
                await bot.db.deleteWebhook(msg.channel.id) //bot.db.query('DELETE FROM Webhooks WHERE channel_id = $1', [msg.channel.id])
                const hook = await bot.fetchWebhook(msg.channel)
                webmsg = await bot.executeWebhook(hook.id, hook.token, data)
    ...
    bot.err = (msg, error, tell = true) => {
        ...
        if (tell && msg.channel)
            bot.send(msg.channel, `There was an error performing the operation. Please report this to the support server if issues persist. (${error.code || error.message})`).catch(e => {})
    ...
    bot.checkMember = (msg, member, clean) => {
        for (let i = 0; i < member.brackets.length / 2; i++) {
            if (
                clean.startsWith(member.brackets[i * 2]) &&
                clean.endsWith(member.brackets[i * 2 + 1]) &&
                ((clean.length == member.brackets[i * 2].length + member.brackets[i * 2 + 1].length && msg.attachments[0]) ||
                    clean.length > member.brackets[i * 2].length + member.brackets[i * 2 + 1].length)
            )
                return i
        }
        return -1
    ...
    bot.fetchWebhook = async channel => {
        let q = (await bot.db.getWebhook(channel.id)) || [] //('SELECT * FROM Webhooks WHERE channel_id = $1', [channel.id])
        if (q[0]) return q[0]
        else if (!channel.permissionsOf(bot.user.id).has('manageWebhooks')) throw new PermissionsError('Manage Webhooks')
        else {
            let hook
            try {
            ...
            await bot.db.createWebhook(hook.id, channel.id, hook.token) //bot.db.query('INSERT INTO Webhooks VALUES ($1,$2,$3)', [hook.id, channel.id, hook.token])
            return wbhk
        }
    }
    ...
    bot.updateStatus = async () => {
        if(bot.presence.game && process.env.HOSTNAME){
            if(bot.presence.game.name.endsWith("registered")){
                bot.editStatus({ name: `tul!help | Hosted by ${process.env.HOSTNAME}` })
                return
            }
        }
        bot.editStatus({ name: `tul!help | ${await bot.db.memberCount()} registered` })
    }
    ...
    bot.generatePages = async (arr, fieldGen, extra = {}) => {
        ...
        embeds.push(current)
        if (embeds.length > 1) {
            for (let i = 0; i < embeds.length; i++) embeds[i].embed.title += ` (page ${i + 1}/${embeds.length}, ${arr.length} total)`
        }
        return embeds
    }
    ...
    bot.paginate = async (msg, data) => {
        ...
        for (let i = 0; i < buttons.length; i++) await bot.addMessageReaction(msg.channel.id, m.id, buttons[i])
        bot.pages[m.id] = {
            user: msg.author.id,
            pages: data,
            index: 0
        ...
        setTimeout(() => {
            ...
            if (!(msg.channel.type == 1)) bot.removeMessageReactions(msg.channel.id, m.id).catch(e => {})
