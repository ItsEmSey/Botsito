...
module.exports = {
    ...
    usage: cfg => [
        'describe <name> [desc] - if desc is specified, change the ' + cfg.lang + "'s describe, if not, simply echo the current one",
    ...
    execute: async (bot, msg, args, cfg) => {
        ...
        //check arguments
        let member = await bot.db.getMember({ user_id: msg.author.id, name: args[0] })
        if (!member) return "You don't have " + article(cfg) + ' ' + cfg.lang + ' with that name registered.'
        if (!args[1])
            return member.description
                ? 'Current description: ' + member.description + '\nTo remove it, try ' + cfg.prefix + 'describe ' + member.name + ' clear'
                : 'No description currently set for ' + member.name
