module.exports = {
    ...
    usage: cfg => [
        "list [user] - Sends a list of the user's registered " +
            cfg.lang +
            "s, their brackets, post count, and birthday (if set). If user is not specified it defaults to the message author. If 'all' or '*' is given, gives a short form list of all tuppers in the server."
    ],
    permitted: () => true,
    execute: async (bot, msg, args, cfg, ng = false) => {
        ...
        if (groups[0] && !ng) {
            let members = bot.db.getMembers(target.id)
            if (!members) return target.id == msg.author.id ? 'You have not registered any ' + cfg.lang + 's.' : 'That user has not registered any ' + cfg.lang + 's.'
            ...
            for (let i = 0; i < groups.length; i++) {
                ...
                let add = await bot.generatePages(
                    members.filter(t => t.group_id == groups[i].id),
                    t => bot.generateMemberField(t),
                    extra
                )
                if (add[add.length - 1].embed.fields.length < 5 && groups[i + 1])
                    add[add.length - 1].embed.fields.push({
                        name: '\u200b',
        ...
        let members = await bot.db.getMembers(target.id) //(await bot.db.query('SELECT * FROM Members WHERE user_id = $1 ORDER BY position', [target.id])).rows
        if (!members) return target.id == msg.author.id ? 'You have not registered any ' + cfg.lang + 's.' : 'That user has not registered any ' + cfg.lang + 's.'
        ...
        let embeds = await bot.generatePages(
            members,
            async t => {
